...
async def get_period_summary(
    period: str = Query(
        "today", description="Period: 'today', 'yesterday', 'week', 'month'"
    ),
    _: None = Depends(check_full_config),
) -> dict[str, Any]:
    """
...
async def get_dashboard_data(
    ...
    try:
        ...
        today_prices = [
            p
            for p in prices
            if datetime.fromisoformat(p["valid_from"].replace("Z", "+00:00"))
            >= today_start
        ]
        today_consumption = [
            c
            for c in consumption
            if datetime.fromisoformat(c["interval_start"].replace("Z", "+00:00"))
            >= today_start
        ]
...
async def get_usage_recommendations(
    hours_ahead: int = Query(
        24, ge=1, le=48, description="Hours ahead to analyze for recommendations"
    ),
) -> dict[str, Any]:
    """
    ...
    try:
        now = datetime.now(UTC)
        prices = await octopus_client.fetch_prices(
            period_from=now, period_to=now + timedelta(hours=hours_ahead)
        )
