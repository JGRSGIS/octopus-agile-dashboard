...
def check_consumption_config():
    ...
    if not settings.MPAN:
        raise HTTPException(
            status_code=503,
            detail="MPAN not configured. Add it to your .env file."
        )
...
async def get_consumption(
    period_from: Optional[datetime] = Query(
        None,
        description="Start of period (ISO format). Defaults to 7 days ago."
    ),
    period_to: Optional[datetime] = Query(
        None,
        description="End of period (ISO format). Defaults to now."
    ),
