...
class ConsumptionStatistics:
    ...
    peak_period: Optional[Dict[str, Any]]
...
def calculate_cost_analysis(
    ...
    for c in consumption:
        ...
        # Find matching price
        price = price_lookup.get(interval_key, None)
...
def get_current_and_upcoming_prices(
    ...
    best_upcoming = sorted(upcoming, key=lambda x: x["value_inc_vat"])[:5] if upcoming else []
