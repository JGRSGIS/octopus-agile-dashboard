...
class OctopusAPIClient:
    ...
    async def fetch_prices(
        ...
        if use_cache:
            ...
            if cached:
                logger.debug(f"Returning cached prices for {period_from} to {period_to}")
                return cached
    ...
    async def fetch_consumption(
        ...
        # Check cache first
        if use_cache:
            cache_key = cache_manager.generate_consumption_cache_key(period_from, period_to)
            cached = await cache_manager.get(cache_key)
            if cached:
                logger.debug(f"Returning cached consumption for {period_from} to {period_to}")
                return cached
    ...
    async def get_current_product_code(self) -> Optional[str]:
