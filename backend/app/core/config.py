...
class Settings(BaseSettings):
    ...
    REGION: str = Field(default="H", description="DNO region code (H = Southern England)")
    
    # Product Configuration (auto-detected if not set)
    PRODUCT_CODE: str = Field(default="AGILE-24-10-01", description="Agile product code")
    ...
    PRICE_FETCH_INTERVAL_MINUTES: int = Field(default=30, description="Price fetch interval")
    ...
    CORS_ORIGINS: List[str] = Field(
        default=["http://localhost:3000", "http://localhost:5173", "http://raspberrypi.local"],
